...
import { useAupdateUserMutation, useGetUserDetailsQuery } from "../../redux/api/userApi";
...
const UpdateUser=()=>{
    ...
    const[aupdateUser, {error, isSuccess}]=useAupdateUserMutation();

    useEffect(()=>{
        if(data?.user){
            setName(data?.user?.name)
            setEmail(data?.user?.email)
            setRole(data?.user?.email)
        }
    },[data])
    ...
    const submitHandler=(e)=>{
        ...
        aupdateUser({id:params?.id, body:userData});
    }
